...
date_color_bg = as_rgb(color_as_int(opts.color7))

ip_color = as_rgb(color_as_int(opts.color4))
ip_hack_color = as_rgb(color_as_int(opts.color2))
...
def get_ip_tun0(ifname):
    ...
    try:
        ...
        return "NOT" # No IP found


def encontrar_valor(valor):
    if isinstance(valor, str):
        ...
        return "Tipo de valor incorrecto. Debe ser un string."
...
def draw_tab(
    ...
    tun0_ip = get_ip_tun0('tun0')
    ip_VPN = encontrar_valor(tun0_ip)
    ip_hack = f" {tun0_ip}  "
    ip = f"   {eth0_ip}  "
    #local_ip = get_local_ip()
    #ip = print(f" {local_ip}")
    cells = get_battery_cells()
#    cells.append((ip_hack_color,ip_VPN))
#    cells.append((ip_color,ip))
    cells.append((clock_color,ip_VPN))
#    cells.append((date_color,ip))
